...
//   // ── Debug logs ──
//   useEffect(() => {
//     if (dentist)  console.log("✅ DENTIST RAW:",  dentist);
//     if (availRaw) console.log("✅ AVAIL RAW:",   availRaw);
//     if (procsRaw) console.log("✅ PROCS RAW:",   procsRaw);
//     if (dentistError)  console.log("❌ DENTIST ERR:",  dentistError);
//     if (availError)    console.log("❌ AVAIL ERR:",    availError);
//     if (procsError)    console.log("❌ PROCS ERR:",    procsError);
//   }, [dentist, availRaw, procsRaw, dentistError, availError, procsError]);
...
//   console.log("🔥 TimeVisit loaded, dentistId:", dentistId);
...
//   // ── ثبت رزرو ──
//   // body: { reservedForId: dentistUserId, reservedProcedureId: procedureId }
//   // header: x-csrf-token (از POST /api/v1/auth/csrf-token)
//   const handleBook = useCallback(async () => {
//     if (!selectedProc || !dentistId) return;
//     setBooking(true);
//     try {
//       // گام ۱: گرفتن CSRF token
//       const csrfData = await apiFetch("/api/v1/auth/csrf-token", { method: "POST" });
//       const csrf = csrfData?.csrfToken ?? csrfData?.token ?? csrfData?.data?.csrfToken ?? "";
 
//       if (!csrf) throw { status: 0, message: "دریافت CSRF token ناموفق بود" };
...
//         headers: { "x-csrf-token": csrf },
//         body: JSON.stringify({
//           reservedForId:       dentistId,       // userId دندانپزشک
//           reservedProcedureId: selectedProc.id, // id خدمت انتخابی
//         }),
//       });
